...
\centering
	...
	\begin{subfigure}{.45\textwidth}
		...
		\begin{tikzpicture}
			\begin{axis}[
				ybar, ymin=0, ymax = 1, symbolic x coords={50, 100, 144, 200}, xtick=data, ylabel=Laufzeit in Stunden, xlabel=$dop$, xlabel near ticks, ylabel near ticks, legend style={ 
					at={(current bounding box.south-|current axis.south)}, anchor=north,
				}]
  			\addplot coordinates {
    				(144, 0.6838)
			...
			\end{axis}
		\end{tikzpicture}}
		\caption{Durchschnittliche Laufzeit der PyFlink und Java-Flink-Implementationen für $dim_k = 500$ und $dim_{tk} = 20$ in Abhängigkeit von $dop$}
