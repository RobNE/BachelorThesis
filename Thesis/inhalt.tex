...
\Function{CutTileIntoSubtiles}{$S, G$}
	...
	\ForAll{$r_k \in R$} \Comment{$nr_{tk}$} \label{alg:iterR}
			...
			\ForAll{$r_{tk} < dim_{tk}$ } \Comment{$dim_{tk}$}  \label{alg:computeS16}
				\State $values_{tk} \leftarrow values_k$ des Zielbereichs \Comment{$dim_{tk}$}
			\EndFor
		\EndFor
...
\centering
	\begin{subfigure}{.45\textwidth}
		...
		\begin{tikzpicture}
			\begin{axis}[
				ybar, ymin=0, ymax = 6, symbolic x coords={10, 20, 50, 100, 250, 500}, ylabel=Laufzeit in Stunden, xlabel=$dim_{tk}$, xlabel near ticks, ylabel near ticks,
  				]
  			\addplot coordinates {
    				(10, 2.3936)
				(20, 0.7861)
			...
			\end{axis}
		\end{tikzpicture}
		\caption{Durchschnittliche Laufzeit der PyFlink und Java-Flink-Implementationen für $dim_k = 500$ und $dop = 144$ in Abhängigkeit von $dim_{tk}$}
	...
	\begin{subfigure}{.45\textwidth}
		...
		\begin{tikzpicture}
			\begin{axis}[
				ybar, ymin=0, ymax = 6, symbolic x coords={50, 100, 144, 200}, ylabel=Laufzeit in Stunden, xlabel=$dop$, xlabel near ticks, ylabel near ticks,
  			]
  			\addplot coordinates {
    				(144, 0.6838)
  			};
			...
			\end{axis}
		\end{tikzpicture}
		\caption{Durchschnittliche Laufzeit der PyFlink und Java-Flink-Implementationen für $dim_k = 500$ und $dim_{tk} = 50$ in Abhängigkeit von $dop$}
  		\label{fig:diagrammFlinkDop}
	\end{subfigure}
\caption{A figure with two subfigures}
\label{fig:diagrammFlink}
\end{figure}
...
\centering
	...
	\begin{subfigure}{.45\textwidth}
		...
		\begin{tikzpicture}
			\begin{axis}[
				ybar, ymin=0, ymax = 6, symbolic x coords={20, 50, 100, 150}, ylabel=Laufzeit in Stunden, xlabel=$n_s$, xlabel near ticks, ylabel near ticks,
  				]	
			\addplot coordinates {
    				(150, 0.65)
  			};	
	...
	\begin{subfigure}{.45\textwidth}
		...
		\begin{tikzpicture}
			\begin{axis}[
				ybar, ymin=0, ymax = 6, symbolic x coords={50, 100, 200, 400}, ylabel=Laufzeit in Stunden, xlabel=$dop$, xlabel near ticks, ylabel near ticks,
  			]
  			\addplot coordinates {
    				(200, 0.65)
				(400, 1.3208)
...
\textcolor{red}{Erkenntnisse Laufzeit für verschiedene $dim_k$}
...
\centering
	...
	\begin{subfigure}{.45\textwidth}
		...
		\begin{tikzpicture}
			\begin{axis}[
				ybar, ymin=0, ymax = 6, symbolic x coords={20, 50, 100, 150, 373}, ylabel=Laufzeit in Stunden, xlabel=$n_s$, xlabel near ticks, ylabel near ticks,
  				]
  			\addplot coordinates {
