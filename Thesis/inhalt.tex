...
\begin{figure}[H]
\centering
\includestandalone[width=12cm]{DataCubeSlicedImage}
...
\begin{algorithm}[H]
	...
	\KwData{\(S\) := \(n\) Szenen \(s_i\) mit \(0 \leq i \leq n\). \(c_1\) und \(c_2\) := Tupel aus Breiten- und Längengrad. \(m_x, m_y\) als Integer, des weiteren gilt (vorerst) \(m_x = m_y\)}
 	...
 	\ForEach{Szene \(s_i\) in \(S\)}{ \label{alg:groupOriginalScenes}
		Gruppierung von \(s_i\) gemäß dem Aufnahmezeitpunkt\;
		}
	\ForEach{Gruppe g in \(Gruppen_{Aufnahmezeitpunkt}\)}{ \label{alg:processGroups}
		\ForEach{Szene \(s_i\) in \(g\)} { \label{alg:cutAllTilesOfAGroup}
	...
	\ForEach{\(Teilkachel_{i_{m_x,m_y}}\)}{ \label{alg:groupSubscenes}
		Gruppierung von \(Teilkachel_{i_{m_x,m_y}}\) gemäß dem Aufnahmedatum und dem Spektralband\;
	}
	Sortierung der Gruppen nach Aufnahmedatum\; \label{alg:sortSubscenes}
	\tcp{Approximierung der fehlenden Vegetationsindexwerte}\
	\ForEach{Gruppe g in \(Gruppen_{Aufnahmezeitpunkt, Band}\)}{ \label{alg:approxVegetationIndex}
		Transformation der einzelnen Teilkacheln in Pixelzeitreihen \label{alg:createPixelTimeSeries}
		Approximierung der fehlenden Werte für jede Pixelzeitreihe \label{alg:approxPixelTimeSeries}
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[>=latex']
		\begin{axis}[xmin=0, xmax=6*pi, ymin=-2, ymax=2, xlabel={Zeit}, ylabel={Beobachtungswerte},legend style={ 
			...
			\addplot[samples=500,domain=0:6*pi, color=red]{sin(deg(x))};
			\addlegendentry{$Approximierte Sinusfunktion$}
		\end{axis}
	\end{tikzpicture}
	\caption{figure}{Prognosemodell für den Vegetationsindex geographisch eindeutig bestimmter Pixel}
\end{figure}
...
Darauf aufbauend wird der Faktor "'Anzahl der Teilkacheln"' getestet, indem für alle Implementationsvarianten bei identischen Eingabedatenmenge verschiedene Werte für die Anzahl der Teilkacheln angenommen wird. Dieser muss \(1 \leq \mathbin{\#}Teilkacheln_{Szene\ s} \leq \mathbin{\#} Pixel_{Szene\ s} \) betragen. 
