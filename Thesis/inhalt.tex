...
\Function{ApproximateVegetationIndex}{$S, G$}
	...
	\ForAll{$S_t $} \label{alg:dataCubeCreation}
		...
		\EndFor
	\EndFor
	\ForAll{$k_{t, band} \in K_{band}$} \label{alg:flatMapOperator}
	...
	\EndFor
	\ForAll{$TK_{pos\_tk, band}$}
		\State $TK_{pos\_tk, band} \leftarrow sortiere \forall tk_{t, pos\_tk, band} \in TK_{pos\_tk, band} nach t$ \label{alg:sortSubscenes}
		\ForAll{$tk_{t, pos\_tk, band} \in TK_{pos\_tk, band}$} \label{alg:createPixelTimeSeriesLoopStart}
			\ForAll{$P_{pos\_px} \in tk_{t, pos\_tk, band}$}
