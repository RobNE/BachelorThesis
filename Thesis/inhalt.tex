...
wenn $nr_{tk} = ns_{tk}$ gilt. $nr_{tk} = ns_{tk}$ folgt aus der im vorigen Abschnitt beschrieben Vorgabe, dass sowohl die Kacheln als auch die Teilkacheln quadratisch sein müssen. Da der Algorithmus für jede Kachel ausgeführt wird, hat die Anzahl der Kacheln keine Bedeutung bezüglich der Komplexität, für die Bestimmung der Komplexität des gesamten Programms wird diese jedoch eine Rolle spielen.
...
\Function{CutTileIntoSubtiles}{$S, G$}
	...
	\State Berechne $ns_{tk}$ als Anzahl der Teilkacheln pro Spalte $s \in S$ von $k_{t, b}$ \Comment{1} \label{alg:calcNR}
...
\textcolor{red}{Auf KomplAna verweisen}
